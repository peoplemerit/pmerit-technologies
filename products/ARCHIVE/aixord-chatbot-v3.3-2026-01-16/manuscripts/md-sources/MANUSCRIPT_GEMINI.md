...
subtitle: "Structured Governance for Google Gemini"
...
## Structured Governance for Google Gemini
...
**Gem** — A custom AI persona in Gemini Advanced with persistent instructions and knowledge.
...
- Gemini knows whether to explore possibilities or execute plans
- Progress is measurable and verifiable

When Gemini displays its response header, you will always see which kingdom you are in. This constant reminder helps maintain the discipline that makes AI collaboration productive.
...
During the ASSESS phase in the Ideation Kingdom, before you can pass the Ideation Gate, Gemini helps you map your project into a DAG. Each deliverable becomes a SCOPE with:
...
The DAG determines execution order. When you enter the Realization Kingdom, Gemini identifies which SCOPEs have no unmet dependencies. These are eligible to start. As each SCOPE is verified complete, new SCOPEs become eligible.
...
Gemini will note these opportunities: "SCOPE_WEB and SCOPE_MOBILE can proceed in parallel. Which would you like to start?"
...
During BRAINSTORM and OPTIONS phases, Gemini evaluates proposed approaches against these dimensions. Each dimension receives a status:
...
Gemini will display a table showing each dimension, its assessment, and any notes. This makes quality visible and discussable rather than implicit and assumed.
...
When Gemini recommends a Priority 4 or 5 solution, it must provide justification: what capability does this provide that lower-priority options cannot match? This prevents defaulting to expensive solutions when simpler alternatives exist.
...
Gemini monitors the conversation and acts at key thresholds:

| Context Level | What Gemini Does |
|---------------|------------------|
...
Gemini will also generate HANDOFFs when:
...
When Gemini generates a HANDOFF:
...
Gemini will guide you through reconstruction:
...
After providing information, Gemini will show a recovery summary for confirmation before continuing.

### Prevention Tips

1. Save HANDOFFs immediately when Gemini generates them
...
### Gemini Advanced (Gem)
1. Create AIXORD Gem (one-time, 5 min)
2. Open Gem → `PMERIT CONTINUE` → Work
...
When you unzip `aixord-gemini-pack.zip`, you'll find several files. Here's what each one does and when to use it.
...
| `README.md` | Getting started guide | Read first — step-by-step setup instructions |

**The governance file is the heart of AIXORD.** This is what transforms Gemini from a general chatbot into your structured project partner.

## Quick-Start Guides (Read These First)

| File | What It Is | Who It's For |
|------|-----------|--------------|
| `AIXORD_GEMINI_ADVANCED.md` | Setup guide for Gems | Gemini Advanced subscribers who want persistent setup |
| `AIXORD_GEMINI_FREE.md` | Setup guide for Free tier | Gemini Free users (paste workflow) |

**⚠️ IMPORTANT:** These guides teach you HOW to use AIXORD. They are instruction manuals for YOU to read.

**Do NOT paste these guide files into Gemini.** Paste `AIXORD_GOVERNANCE_GEMINI_V3.3.md` instead.

## Legal Files (Keep for Reference)

| File | What It Is |
|------|-----------|
| `LICENSE.md` | Your usage rights and terms |
| `LICENSE_KEY.txt` | Your license certificate (proof of purchase) |
| `DISCLAIMER.md` | Legal disclaimer and limitations |
...
```
Which file do I paste into Gemini?
│
├─► ALWAYS paste: AIXORD_GOVERNANCE_GEMINI_V3.3.md
│
└─► NEVER paste: AIXORD_GEMINI_ADVANCED.md or AIXORD_GEMINI_FREE.md
                 (These are guides for YOU to read, not for Gemini)
```
...
When you first activate AIXORD (after license validation), Gemini will ask:
...
**Verification Required:** After creating this structure, you'll upload a screenshot. Gemini will verify each folder exists before proceeding. This ensures you're set up for success.

**What Gemini Checks:**
- All 6 numbered folders present
- Correct naming convention
- Governance file in correct location

If anything is missing, Gemini will tell you exactly what to fix and ask for a new screenshot.
...
- Gemini reminds you to save HANDOFFs but doesn't verify
- You're responsible for maintaining organization
- Recovery from lost sessions may be harder

**Required Acknowledgment:** You must type "YES" to confirm you understand the responsibility.
...
|--------|---------|----------------|
| `1_GOVERNANCE/` | AIXORD rules | The governance file (copy from ZIP) |
| `2_STATE/` | Progress tracking | AIXORD_STATE.json (copy from ZIP, update as you work) |
| `3_PROJECT/` | Your specs | PROJECT_DOCUMENT.md (your project definition) |
| `4_HANDOFFS/` | Session saves | All HANDOFF files (named by date) |
| `5_OUTPUTS/` | Deliverables | Final work products |
| `6_RESEARCH/` | Reference | Research, notes, resources |

---

### Creating the PROJECT_DOCUMENT.md

Your PROJECT_DOCUMENT.md should contain:
...
## Constraints
- [Time limits, budget, technical requirements]

## Notes
[Anything else Gemini should know]
```

**Tip:** The more detail you provide, the better Gemini can help you.

---

### For Gemini Advanced Users (Gems)

If you're using the Gem workflow (Tier A):

1. Create your project folder structure first
2. When setting up your Gem:
   - Put governance in **Instructions**
   - Upload your entire project folder to **Knowledge**
3. Gemini can then access all your files directly

This makes the Absolute structure especially valuable—everything is organized for easy upload.

---

### Changing Your Choice Later

Your folder structure preference is saved in `AIXORD_STATE.json`. If you want to switch:

1. Update the `folder_structure` field in your STATE file
2. Set `folder_verified` to `false`
3. Start a new session—Gemini will re-run verification if needed

---

### Quick Reference: Folder Setup Commands

| Situation | What to Do |
|-----------|------------|
| First time setup | Wait for Gemini prompt after license validation |
| Already verified | Gemini skips—you're good to go |
| Want to re-verify | Update STATE.json, set `folder_verified: false` |
| Switching approaches | Update `folder_structure` field in STATE.json |
...
|-------------------------------|-------------|--------|
| `# AIXORD GOVERNANCE` | ✅ Governance file | Paste this into Gemini |
| `# AIXORD for Gemini Advanced` | ❌ Quick-start guide | Don't paste — read it instead |
| `# AIXORD for Gemini Free` | ❌ Quick-start guide | Don't paste — read it instead |

The governance file is approximately 18-20 KB. The quick-start guides are smaller (5-10 KB).
...
1. Create your AIXORD Gem (see Chapter 6, Part A)
2. Open the Gem from the sidebar

**Gemini Free Users:**
1. Open gemini.google.com → New Chat
2. Paste the activation template + governance (see Chapter 6, Part B)
...
Gemini will ask for your license email (first time only), detect your tier, and ask what you want to work on.
...
Gemini will ask discovery questions to surface your project from frustrations, wishes, and repetitive tasks.

## Step 4: Brainstorm

Once you have a project direction, Gemini enters BRAINSTORM mode:
...
Answer each question. Gemini documents your answers.

## Step 5: Options

After brainstorming, Gemini presents 2-3 solution approaches:
...
Gemini generates your PROJECT DOCUMENT with:
...
With the document approved, Gemini guides implementation:

- Tier A (Gem): Gemini provides guidance with persistent context
- Tier B (Advanced): Gemini provides step-by-step instructions
- Tier C (Free): Gemini guides you through manual execution
...
Gemini generates a handoff document. Save it for your next session.
...
4. Gemini will reconstruct context and confirm before continuing
...
│  Architect (Gemini)  → Plans and specifies                  │
...
**Product:** AIXORD Gemini Pack
**Regular Price:** $7.99
**Your Price:** FREE (with code)
**URL:** https://meritwise0.gumroad.com/l/qndnd

## How to Download

1. Visit the URL above (or type it into your browser)
2. Click "I want this!"
3. Enter the discount code: AX-GEM-6R4T
...
| **AIXORD_GEMINI_ADVANCED.md** | Setup guide for Gemini Advanced |
