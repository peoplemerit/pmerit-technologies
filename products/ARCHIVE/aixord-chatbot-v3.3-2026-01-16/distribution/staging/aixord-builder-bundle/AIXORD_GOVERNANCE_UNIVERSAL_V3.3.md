...
When AI generates a HANDOFF document, it MUST be **completely self-contained**. The receiving AI instance (whether the same platform or different) must be able to understand and continue work with ZERO external context.

### WHY SELF-CONTAINED

| Problem | Solution |
|---------|----------|
| New AI session has no memory | HANDOFF includes ALL relevant context |
| User may switch AI platforms | HANDOFF is platform-agnostic |
| Project files may not be accessible | HANDOFF embeds key decisions and specs |
| Long sessions degrade context | HANDOFF captures clean state |
...
## 1. PROJECT CONTEXT
[Complete project description — assume reader knows NOTHING]
- What is being built
...
## 2. GOVERNANCE CORE
[Embedded governance rules — the receiving AI needs these]
- Authority model (Director/Architect/Commander)
...
- Completed SCOPEs (with verification status)
- Active SCOPE (with progress %)
- Pending SCOPEs (with dependencies)
- Blockers (if any)

## 4. DECISIONS MADE
[All significant decisions from this and previous sessions]
...
[Explicit next steps]
- Immediate next task
- Required inputs from Director
- Known blockers to address

## 6. KEY ARTIFACTS
[Reference to or embedding of critical documents]
- MASTER_SCOPE summary
- Active SCOPE details
- Any specifications needed to continue

## 7. SESSION NOTES
[Context that would be lost]
- Conversations that influenced decisions
- Clarifications received from Director
- Assumptions made (and their status)
```

### HANDOFF GENERATION CHECKLIST

Before generating HANDOFF, AI MUST verify:

| Check | Requirement |
|-------|-------------|
| Context Complete | Could a new AI understand the project from this document alone? |
| State Accurate | Does the state reflect actual progress, not planned progress? |
| Decisions Documented | Are all significant decisions recorded with rationale? |
| Next Steps Clear | Is the immediate next action unambiguous? |
| Artifacts Included | Are key specs embedded or clearly referenced? |
| Blockers Listed | Are any blockers explicitly stated? |

### GOVERNANCE CORE TEMPLATE

When embedding governance in HANDOFF, include this minimum set:

```markdown
### GOVERNANCE CORE (AIXORD v3.3.1)
...
Every AI response must begin with:
┌──────────────────────────────────────┐
│ Phase: [PHASE]                        │
│ Task: [Current task]                  │
│ Progress: [X/Y]                       │
...
- IDEATION GATE: Blocking checkpoint (requires FINALIZE PLAN)
...
4. Expansion Triggers — Only expand when Director requests
...
- HANDOFF: Full handoff, end session
- HALT: Stop and return to Director
- FINALIZE PLAN: Attempt Ideation Gate passage

**Enforcement:**
- HALT on ambiguity
- HALT on scope deviation
- HALT on unmet DAG dependencies
- HALT on blocked Ideation Gate
```

### VALIDATION

After generating HANDOFF, AI must self-validate:

```
HANDOFF VALIDATION:
[ ] Project context understandable without prior knowledge
[ ] Governance rules embedded (not just referenced)
[ ] State reflects reality (verified, not assumed)
[ ] All decisions documented with rationale
[ ] Next action is specific and actionable
[ ] Key artifacts included or embedded
[ ] No assumptions left undocumented
```

If any check fails, AI must fix before presenting HANDOFF to Director.
