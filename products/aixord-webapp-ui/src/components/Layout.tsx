...
export function Layout() {
  ...
  // Hide header and footer on project pages — Project.tsx has its own TabBar + StatusBar
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      {/* Header — hidden on Project pages (Project.tsx has its own TabBar) */}
      {!isProjectPage && (
      ...
      {/* Sidebar — hidden on Project pages (Project.tsx uses full-width layout) */}
      {isAuthenticated && !isProjectPage && (
        <Sidebar
          isOpen={true}
      ...
      {/* Main Content — no margin on Project pages since sidebar is hidden */}
      <main className={`
        transition-all duration-300
        ${isAuthenticated && !isProjectPage ? (sidebarCollapsed ? 'ml-16' : 'ml-56') : ''}
      `}>
        <Outlet />
      </main>

      {/* Footer — hidden on Project pages (StatusBar serves as bottom anchor) */}
      {!isProjectPage && (
